...
describe("prepareReadOnlyQuery", () => {
  ...
  it("appends a custom limit", () => {
    expect(prepareReadOnlyQuery("SELECT * FROM users", 50)).toBe(
  ...
  it("leaves a trailing LIMIT untouched", () => {
    expect(prepareReadOnlyQuery("SELECT * FROM t LIMIT 10")).toBe(
  ...
  it("strips trailing semicolons before appending", () => {
    expect(prepareReadOnlyQuery("SELECT 1;")).toBe(
      `SELECT 1 LIMIT ${DEFAULT_QUERY_LIMIT}`,
    );
  });
  ...
  it("passes EXPLAIN through untouched", () => {
    expect(prepareReadOnlyQuery("EXPLAIN SELECT * FROM t")).toBe(
      "EXPLAIN SELECT * FROM t",
    );
  });

  it("does not append LIMIT after FOR UPDATE (syntax error)", () => {
    expect(prepareReadOnlyQuery("SELECT * FROM t FOR UPDATE")).toBe(
