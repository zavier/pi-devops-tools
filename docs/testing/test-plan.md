...
### D. 写操作(db_mutate, 默认全部需人工确认; D14 会话开关例外)
...
| D14 | **会话免确认开关**  | `/db auto-approve on` 后再触发任意 INSERT                                   | 开关弹二次确认; 开启后状态栏连接信息出现 ` • 免确认` 后缀, 写操作**不弹窗**且结果标注「免人工确认」; `/db auto-approve off` 或 `/reload` 后后缀消失、恢复弹窗 | 关闭开关 + 按对应用例还原数据                                             |
...
| G6b | 样例数据         | `/db sample` → 选 t_orders                                                | 先弹可搜索表列表; 选中后渲染 10 行样例数据(表头含 `LIMIT 10`), 与 `/db query` 同样的自适应表格                                                                    |
