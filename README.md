...
| `/db sample [表名]`                     | 查看样例数据（固定 10 行）；交互同 `/db schema`：不带参数先弹可搜索的表列表                                       |
...
只想快速看一眼某个表的数据时用 `/db sample [表名]`：固定取 10 行（`SELECT * FROM \`表名\` LIMIT 10`），与查询共用同一个只读执行路径和结果渲染。
...
| 文件               | 用途                                                |
| ------------------ | --------------------------------------------------- |
| `workspaces.json`  | 当前环境/数据库选择，按项目目录（cwd）隔离          |
| `state.db`         | 查询历史、收藏、表关系的 SQLite 存储                |
| `connections.yaml` | 连接定义                                            |
| `extension.json`   | 扩展开关状态（`{ "enabled": bool }`，缺失默认启用） |

选择状态按项目隔离：在哪个项目里 `/db switch` 过，下次进入该项目会自动恢复；其他项目不受
影响。升级自 0.9.x 时，旧的全局 `workspace.json` 会被改名为 `workspace.json.legacy` 留档
（不继承其值，每个项目重选一次）。

未选择数据库时，扩展不会向模型注入任何提示——"没有数据库"只展示在 `/db` 面板和状态栏，不
占用上下文；模型真要用库时会自己撞到错误提示，并可改用 `db_discover` + 显式参数。
